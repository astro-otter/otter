...
from otter.util import FILTER_MAP_WAVE, FILTER_MAP_FREQ, bibcode_to_hrn
...
def upload_zip(outdir: str, zipfile: str, testing: bool = False) -> None:
    ...
    otter = Otter(datadir=outdir, gen_summary=True)
...
def _row_to_json(dfrow: pd.Series, datapath: str, testing=False) -> None:
    ...
    galactic = SkyCoord(
        dfrow.ra, dfrow.dec, unit=(dfrow.ra_units, dfrow.dec_units), frame="icrs"
    ).galactic
    ...
    if not testing:  # we don't want to use up all our queries
        ...
        schema["reference_alias"] = [
            {"name": bibcode, "human_readable_name": bibcode_to_hrn(bibcode)}
        ]
    else:
