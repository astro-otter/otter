...
def set_bot_tns_marker():
    tns_marker = 'tns_marker{"tns_id": ' + str(os.environ['TNS_BOT_ID']) + ', "type": "bot", "name": "' + str(os.environ['TNS_BOT_NAME']) + '"}'
    return tns_marker
...
def search(ra:str, dec:str, radius:str='10', radius_units:str="arcsec",
...
def get(objname:str, objid:int, phot:str="1", spec:str="0", test:bool=False):
    ...
    get_obj = [("objname", objname), ("objid", objid), ("photometry", phot), ("spectra", spec)]
...
def download_daily_csv(outfile='tns_public_objects.csv'):
    ...
    if response.status_code == 200:
        ...
        raise ValueError(f'Downloading the TNS Daily CSV Failed with Code {response.status_code}')
...
def search_tns_daily_csv(db:Otter, tnsdaily:str, sep=5) -> pd.DataFrame:
    ...
    tns_idx, otter_idx, _, _ = search_around_sky(tns_skycoords, otter_skycoords, sep*u.arcsec)
    ...
    alldata = pd.concat([tns_df_matches, 
                         otter_df_matches], 
                        axis=1
             )
...
def tns_phot_to_otter_phot(photlist):
    ...
    for point in photlist:
        ...
        otter_format = dict(
            ...
            reference = 'TNS', # CORRECT THIS LATER? I'M NOT SURE HOW TO GET THE ASSOCIATED BIBCODE
...
def tns_response_to_otter(row, photlist):
    ...
    if not pd.isna(row.Class_ADS_bibcodes_tns):
        if not pd.isna(row.redshift_tns):
            out['distance'] = [dict(
                ...
                reference = [b.strip() for b in row.Class_ADS_bibcodes_tns.split(',')]
            )]
    ...
    if not pd.isna(row.Class_ADS_bibcodes_tns):
        for bib in row.Class_ADS_bibcodes_tns.split(','):
            toadd = dict(
                name=bib,
                human_readable_name=bibcode_to_hrn(bib)
            )
...
def main():
    ...
    p.add_argument('--sep', help='sky separation for comparing to otter in arcsec',
