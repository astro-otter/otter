...
# Summary
The Open mulTiwavelength Transient Event Repository (OTTER) is a new catalog of
...
also provides additional methods for converting the stored photometry to standard
units; helper methods for querying additional astronomy database services; and
...
data. However, the API endpoints expect queries in the syntax of the ArangoDB Query
Language (AQL). Learning a new query language creates a barrier for programmatic
...
Evidence of this is the impact of the Open Astronomy Catalogs, which have $>500$ citations
...
groups have already used OTTER for writing successful telescope observing proposals. We presented
...
# Acknowledgements
NF acknowledges support from the National Science Foundation Graduate Research Fellowship
